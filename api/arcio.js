...
module.exports.load = async function (app, db) {
  app.get("/arc-sw.js", async (req, res) => {
    try {
      ...
      const newsettings = JSON.parse(await fs.readFile(settingsPath, "utf8"));

      if (newsettings.api.arcio.enabled) {
        res.type(".js");
  ...
  app.get("/arcioerror", async (req, res) => {
    ...
    const redirectUrl = theme.settings.redirect.arcioerror + (req.query.err ? `?arcioerr=${req.query.err}` : "");
    res.redirect(redirectUrl);
  });

  app.ws("/" + settings.api.arcio["afk page"].path, async (ws, req) => {
    try {
      await handleWebSocket(ws, req, db);
    } catch (error) {
...
async function handleNotFound(req, res) {
  try {
    ...
    ejs.renderFile(
      path.join(__dirname, "..", "themes", theme.name, theme.settings.notfound),
      renderData,
      null,
      (err, str) => {
...
async function handleWebSocket(ws, req, db) {
  if (!req.session.arcsessiontoken) return ws.close();

  const token = req.headers["sec-websocket-protocol"];

  if (!token || typeof token !== "string" || token !== req.session.arcsessiontoken) {
    return ws.close();
  }

  const newsettings = JSON.parse(await fs.readFile(path.join(__dirname, "..", "settings.json"), "utf8"));

  if (!newsettings.api.arcio.enabled || !newsettings.api.arcio["afk page"].enabled || currentlyonpage[req.session.userinfo.id]) {
    return ws.close();
  }
